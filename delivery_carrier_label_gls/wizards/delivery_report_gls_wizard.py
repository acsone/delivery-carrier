...
class DeliveryReportGls(models.TransientModel):
    ...
    @api.model
    def cron(self):
        return self.create({})._get_end_of_day_report(False)

    def get_end_of_day_report(self, date=False):
        self.ensure_one()
        reports = self._get_end_of_day_report(date)
        if not reports:
    ...
    def _get_end_of_day_report(self, date=False):
        reports = self.env["delivery.report.gls"].browse()
        for carrier in self.carrier_ids:
            # TODO: actually process each login only once.
            reports |= self._get_carrier_end_of_day_report(carrier, date)
        return reports

    def _get_carrier_end_of_day_report(self, delivery_carrier, date=False):
